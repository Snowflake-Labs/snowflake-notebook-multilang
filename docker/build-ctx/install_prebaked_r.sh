...
if [[ "${SFNB_CRE_ADBC}" == "1" ]]; then
  ...
  "${MM_BIN}" install -y -n "${ENV_NAME}" -c "${CONDA_CHANNEL}" "${ADBC_CONDA_PACKAGES[@]}"
...
if (!requireNamespace('adbcdrivermanager', quietly = TRUE)) {
  install.packages('adbcdrivermanager', repos = cran, quiet = TRUE)
}
if (!requireNamespace('adbcsnowflake', quietly = TRUE)) {
  install.packages('adbcsnowflake', repos = 'https://community.r-multiverse.org', quiet = TRUE)
}
